    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .padding(.vertical, 6)
                .background(isDestructive ? Color.red.opacity(0.85) : FaceIDTheme.accent)
                .clipShape(Capsule())
        }
        .buttonStyle(.plain)
        .disabled(!isEnabled)
            ...
            VStack(spacing: 8) {
                preview()
                    ...
                    .background {
                        ...
                        if isSelected {
                            FaceIDTheme.accent.opacity(SettingsMetrics.optionPreviewSelectedTintOpacity)
                        }
                    }
...
final class WindowConfiguringView: NSView {
    ...
    private static func configure(_ window: NSWindow) {
        ...
        let toolbar = NSToolbar(identifier: "NotchPulseFaceIDSettingsToolbar")
