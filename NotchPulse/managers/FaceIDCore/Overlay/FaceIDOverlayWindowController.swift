//
//  FaceIDOverlayWindowController.swift
//  NotchPulse
//
...
final class FaceIDOverlayWindowController {
    ...
    /// top-down coordinates — reported by `FaceIDOverlayView`.
    ...
    /// The SwiftUI content to host — set once by FaceIDOverlayController.
    ...
    /// see `FaceIDOverlayController.primeWindowIfNeeded`.
    ...
    /// Forwarded from `FaceIDOverlayView` every time the visible panel's frame changes.
    func setInteractiveContentRect(_ rect: CGRect?) {
        interactiveContentRect = rect
        updateMousePassthrough()
    }

    var currentGeometry: FaceIDOverlayGeometry {
        FaceIDOverlayGeometry.preferredScreen().map(FaceIDOverlayGeometry.forScreen) ?? FaceIDOverlayGeometry.forMainScreen()
    }
    ...
    private func windowIfNeeded() -> FaceIDOverlayWindow {
        ...
        // Never resized afterward (see FaceIDOverlayWindow.swift), so a style change
        // mid-session keeps whatever margin it was created with.
        let size = FaceIDOverlayGeometry.windowSize(for: currentGeometry.style)
    ...
    private func reposition(_ window: FaceIDOverlayWindow) {
        guard let screen = FaceIDOverlayGeometry.preferredScreen() else { return }
