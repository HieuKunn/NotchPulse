//
//  NotchPulseEnrollmentService.swift
//  NotchPulseFaceID
//
...
final class NotchPulseEnrollmentService {
    ...
    private let embedder: NotchPulseFaceEmbedder?
    private let embedderLoadError: Error?
    private let ciContext = CIContext()

    init() {
        do {
            self.embedder = try NotchPulseFaceEmbedder()
    ...
    func detectFacePosition(in pixelBuffer: CVPixelBuffer) throws -> CGRect {
        ...
        guard !inROI.isEmpty else { throw NotchPulseEnrollmentError.noFaceDetected }
        guard inROI.count == 1 else { throw NotchPulseEnrollmentError.multipleFacesDetected }
    ...
    func analyzeFrame(_ pixelBuffer: CVPixelBuffer, includeQuality: Bool = true) throws -> FrameAnalysis {
        guard let embedder = embedder else {
            throw NotchPulseEnrollmentError.modelUnavailable(
        ...
        guard face.boundingBox.width >= Self.minimumFaceWidthFraction else {
            throw NotchPulseEnrollmentError.faceTooSmall
        }
        ...
        do {
            ...
            throw NotchPulseEnrollmentError.embeddingFailed(underlying: error)
        }
    }
    ...
    func saveEmbeddings(_ embeddings: [[Float]]) throws -> URL {
        do {
            ...
            let encrypted = try NotchPulseVault.encryptWithSessionKey(json)
            ...
            throw NotchPulseEnrollmentError.storageFailed(underlying: error)
        }
    }
    ...
    func verify(
        ...
        threshold: Float = NotchPulseEnrollmentService.defaultMatchThreshold
    ) throws -> VerificationResult {
        guard let saved = try Self.loadEmbeddings(), !saved.isEmpty else {
            throw NotchPulseEnrollmentError.notEnrolled
        }

        let centroid = Self.computeCentroid(saved)
        let centroidSim = NotchPulseFaceEmbedder.cosineSimilarity(currentEmbedding, centroid)

        var maxSim: Float = -1
        for emb in saved {
            let sim = NotchPulseFaceEmbedder.cosineSimilarity(currentEmbedding, emb)
    ...
    private static func computeCentroid(_ embeddings: [[Float]]) -> [Float] {
        ...
        return NotchPulseFaceEmbedder.l2Normalize(centroid)
    }
    ...
    private func cropFace(in pixelBuffer: CVPixelBuffer, boundingBox: CGRect) throws -> CGImage {
        ...
        guard !pixelRect.isEmpty else { throw NotchPulseEnrollmentError.croppingFailed }

        let cropped = ciImage.cropped(to: pixelRect)
        guard let cgImage = ciContext.createCGImage(cropped, from: cropped.extent) else {
            throw NotchPulseEnrollmentError.croppingFailed
        }
        return cgImage
    }
    ...
    static func storageDirectory() throws -> URL {
        ...
        let dir = appSupport.appendingPathComponent("NotchPulseFaceID", isDirectory: true)
