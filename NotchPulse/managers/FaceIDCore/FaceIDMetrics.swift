...
enum FaceIDMetrics {
    ...
    static func panelHeight(for step: FaceIDOnboardingStep, style: NotchPanelStyle) -> CGFloat {
        switch (step, style) {
    ...
    static func panelSize(for step: FaceIDOnboardingStep, style: NotchPanelStyle) -> CGSize {
        let width = step == .enroll ? enrollPanelWidth : panelWidth
        return CGSize(width: width, height: panelHeight(for: step, style: style))
    }

    static func panelBottomRadius(for step: FaceIDOnboardingStep) -> CGFloat {
        switch step {
    ...
    /// the slowest streak in `FaceIDDirectionSweep.makeSpecs()` plus a small buffer.
...
private struct OnboardingContentHorizontalPadding: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .padding(.leading, FaceIDMetrics.contentLeadingInset(for: style))
            .padding(.trailing, FaceIDMetrics.contentTrailingInset(for: style))
    }
}
...
private struct OnboardingContentPadding: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .padding(.top, FaceIDMetrics.titleTopInset(for: style))
            .padding(.bottom, FaceIDMetrics.contentBottomInset(for: style))
    }
}
