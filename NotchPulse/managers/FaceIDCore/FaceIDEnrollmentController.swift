...
/// `String`-backed so `NotchPulseFaceIDSettings.onboardingResumeStep` can persist it directly by name.
enum FaceIDOnboardingStep: String, CaseIterable {
    case intro
    case permissions
    case securityNotice
    case preSetup
    ...
    var previous: FaceIDOnboardingStep? {
    ...
    var resumeTarget: FaceIDOnboardingStep {
        switch self {
...
enum FaceIDEnrollmentPose: Int, CaseIterable {
...
final class FaceIDEnrollmentController {
    ...
    private let sweepWindow = FaceIDDirectionSweepWindowController()
    ...
    private(set) var step: FaceIDOnboardingStep = .intro {
        didSet {
            guard isFirstRunFlow else { return }
            if step == .complete {
                NotchPulseFaceIDSettings.shared.hasCompletedOnboarding = true
                ...
                NotchPulseFaceIDSettings.shared.hasAcknowledgedSecurityNotice = true
                NotchPulseFaceIDSettings.shared.onboardingResumeStep = nil
            } else {
                NotchPulseFaceIDSettings.shared.onboardingResumeStep = step.resumeTarget
            }
        }
    }
    ...
    /// Which way the step just changed — read by FaceIDOnboardingNotchView to
    ...
    static func startFlow(resumingAt step: FaceIDOnboardingStep? = nil, onFirstRunComplete: (() -> Void)? = nil) {
    ...
    static func startPasswordOnly() {
        Task { @MainActor in
            let controller = FaceIDEnrollmentController(isPasswordOnly: true)
            FaceIDOverlayController.shared.presentOnboarding(controller)
        }
    }
    ...
    static func startPostUpdateNotice(onComplete: (() -> Void)? = nil) {
        let controller = FaceIDEnrollmentController(isPostUpdateNotice: true)
    ...
    var panelSize: CGSize { FaceIDMetrics.panelSize(for: step, style: currentPanelStyle) }
    var panelBottomRadius: CGFloat { FaceIDMetrics.panelBottomRadius(for: step) }

    // MARK: - Permissions

    private(set) var accessibilityGranted = false
    private(set) var cameraPermission: NotchPulseCameraPermission = .notDetermined
    ...
    /// Sectors already captured — read by FaceIDEnrollmentRingView to decide which
    /// ticks are lit.
    private(set) var capturedPoses: Set<FaceIDEnrollmentPose> = []
    /// Bumped every time `.center` is captured; FaceIDEnrollmentRingView observes
    ...
    var currentPose: FaceIDEnrollmentPose? {
        FaceIDEnrollmentPose(rawValue: currentPoseIndex)
    }
    ...
    struct HeadTurn: Equatable {
        /// Compass angle (0 = up, clockwise) — same frame as `FaceIDEnrollmentPose.compassAngle`.
        let angle: Double
    ...
    var overallEnrollmentProgress: Double {
        let total = Double(FaceIDEnrollmentPose.allCases.count * samplesPerPose)
    ...
    func advance() {
        ...
        withAnimation(FaceIDMetrics.stepAnimation) {
    ...
    func back() {
        ...
        default:
            ...
            withAnimation(FaceIDMetrics.stepAnimation) { step = previous }
            if step == .permissions {
    ...
    private func processEnrollFrame() async {
        ...
        let outcome = await Task.detached(priority: .userInitiated) {
            do {
                ...
                guard let face = NotchPulseFaceRecognitionPipeline.largestFace(in: faces) else {
                ...
                NSLog("[FaceIDEnrollment] Recognition pipeline error: %@", error.localizedDescription)
                return EnrollFrameOutcome.noFace
            }
        }.value
    ...
    private func processMatchedEnrollFrame(
        ...
        if capturedForCurrentPose >= samplesPerPose {
            ...
            poseStartedAt = .now
            poseHoldStartedAt = nil
            if currentPoseIndex >= FaceIDEnrollmentPose.allCases.count {
    ...
    private func finishEnrollment() async {
        ...
        sweepWindow.dismiss()
        try? await Task.sleep(for: .seconds(FaceIDMetrics.guideOverlayFadeOut))

        cameraPreviewVisible = false
        try? await Task.sleep(for: .seconds(FaceIDMetrics.previewFadeOut))

        try? await Task.sleep(for: .seconds(FaceIDMetrics.checkmarkDelay))
        showCheckmark = true

        let elapsed = FaceIDMetrics.guideOverlayFadeOut + FaceIDMetrics.previewFadeOut + FaceIDMetrics.checkmarkDelay
        let remaining = max(FaceIDMetrics.cameraCompleteToNameDelay - elapsed, 0)
    ...
    func confirmName() {
        ...
        guard isEnrollmentOnly else {
            navDirection = .forward
            withAnimation(FaceIDMetrics.stepAnimation) { step = .password }
            return
        }
        ...
        navDirection = .forward
        withAnimation(FaceIDMetrics.stepAnimation) { step = .complete }
        scheduleCompletionDismiss()
    }
    ...
    /// `finish(password:)` has called `NotchPulseVault.unlockSession`.
    ...
    private func scheduleCompletionDismiss() {
        Task { [weak self] in
            try? await Task.sleep(for: .seconds(FaceIDMetrics.completeScreenDismissDelay))
