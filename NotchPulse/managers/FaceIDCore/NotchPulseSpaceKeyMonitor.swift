//
//  NotchPulseSpaceKeyMonitor.swift
//  NotchPulse
//
...
//
//  NotchPulse never appears under Input Monitoring: TCC resolves that gate against Accessibility first, and NotchPulse already requires
...
@MainActor
final class NotchPulseSpaceKeyMonitor {
    /// Traces the Input Monitoring handshake, otherwise invisible since TCC decisions happen out of process.
    static let log = Logger(subsystem: "com.hieukunn.notchpulse", category: "inputmonitoring")
    ...
    /// True when launched by Xcode's Run button, which makes TCC decisions attribute to Xcode, not NotchPulse — test permission
    /// behavior from an independently launched copy instead (`open /path/to/NotchPulse.app`, or a build in /Applications).
    ...
    func start() {
        ...
        IOHIDManagerRegisterInputValueCallback(mgr, { context, _, _, value in
            ...
            let monitor = Unmanaged<NotchPulseSpaceKeyMonitor>.fromOpaque(context).takeUnretainedValue()
